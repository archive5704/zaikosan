(() => {
  ...
  async function init(){
    ...
    try{
      ...
      await loadData();
      setStatus('Supabaseに接続中。URLを知っている人は全員、閲覧・追加・修正・削除できます。', 'ok');
      setupRealtime();
    }catch(error){
      console.error(error);
  ...
  function setupRealtime(){
    ...
    const reload = debounce(() => loadData().catch(console.error), 350);
  ...
  function renderShellDate(){
    $('today').textContent = '今日：' + new Intl.DateTimeFormat('ja-JP',{year:'numeric',month:'long',day:'numeric',weekday:'long'}).format(new Date());
    ...
    return [...state.items]
      ...
      .sort((a,b) => {
        if(sort === 'pinned'){
          ...
          return stockNumber(a.stock) - stockNumber(b.stock) || a.reading.localeCompare(b.reading,'en');
        }
        if(a.pinned !== b.pinned) return a.pinned ? -1 : 1;
        if(sort === 'stock') return stockNumber(a.stock) - stockNumber(b.stock) || a.reading.localeCompare(b.reading,'en');
        if(sort === 'reading') return a.reading.localeCompare(b.reading,'en');
        if(sort === 'category') return categoryNameById(a.category_id).localeCompare(categoryNameById(b.category_id),'ja') || stockNumber(a.stock) - stockNumber(b.stock);
        return a.name.localeCompare(b.name,'ja');
      });
  }
  function renderBuyList(){
    const items = [...state.items].filter(isBuyTarget).sort((a,b) => stockNumber(a.stock) - stockNumber(b.stock) || a.reading.localeCompare(b.reading,'en'));
    $('buyCount').textContent = items.length + '件';
    $('buyList').innerHTML = items.length ? items.map(item => '<div class="buyItem"><div class="buyName">' + (item.pinned ? '<span class="pinMark">★</span>' : '') + safe(item.name) + '</div><div class="buyMeta">' + safe(categoryNameById(item.category_id)) + '</div><div class="stock ' + stockClass(item.stock) + '">' + safe(item.stock) + '</div></div>').join('') : '<div class="buyEmpty">買うものはありません</div>';
  }
  function renderItems(){
    const items = visibleItems();
    $('count').textContent = items.length + '件';
    $('empty').style.display = items.length ? 'none' : 'block';
    $('list').innerHTML = items.map(item => '<article class="item"><div class="itemInfo"><h3 class="name">' + (item.pinned ? '<span class="pinMark">★</span>' : '') + safe(item.name) + '</h3><div class="catMini">' + safe(categoryNameById(item.category_id)) + '</div><div class="dateMini">' + formatDate(item.updated_at) + '</div></div><div class="rightPack"><div class="stock ' + stockClass(item.stock) + '">' + safe(item.stock) + '</div><div class="miniActions"><button class="ghost miniBtn" type="button" data-action="edit-item" data-id="' + item.id + '">修正</button><button class="danger miniBtn" type="button" data-action="delete-item" data-id="' + item.id + '">削除</button></div></div></article>').join('');
  }
  ...
  function bindEvents(){
    ...
    $('stockMainBtns').addEventListener('click', event => { const button = event.target.closest('[data-stock-main]'); if(!button) return; stockMain = button.dataset.stockMain; renderStockButtons(); });
    $('stockFracBtns').addEventListener('click', event => { const button = event.target.closest('[data-stock-frac]'); if(!button) return; stockFrac = stockFrac === button.dataset.stockFrac ? '' : button.dataset.stockFrac; renderStockButtons(); });
