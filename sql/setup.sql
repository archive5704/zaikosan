...
select 'ふじもと醤油', 'fujimoto shoyu', '1', id, '', 'もっピー', true from public.categories where name = '調味料'
union all
select '純米調理酒', 'junmai chorishu', '0', id, '', 'もっピー', false from public.categories where name = '調味料'
union all
select 'オリーブ油', 'olive oil', '1/2', id, '', 'あきと', false from public.categories where name = '調味料'
union all
select 'マスタード', 'mustard', '0', id, '', 'あきと', false from public.categories where name = '調味料'
union all
select '薄力粉', 'hakurikiko', '1/4', id, '', 'もっピー', false from public.categories where name = '粉物'
union all
select '片栗粉', 'katakuriko', '1', id, '', 'あきと', false from public.categories where name = '粉物'
union all
select 'にんじん', 'ninjin', '2', id, '', 'もっピー', false from public.categories where name = '野菜'
union all
select '玉ねぎ', 'tamanegi', '1', id, '', 'あきと', false from public.categories where name = '野菜'
union all
select '黒こしょう', 'kuro kosho', '1/4', id, '', 'もっピー', true from public.categories where name = 'スパイス'
union all
select 'カレー粉', 'curry powder', '3/4', id, '', 'あきと', false from public.categories where name = 'スパイス'
union all
select '昆布', 'kombu', '0', id, '', 'あきと', false from public.categories where name = '乾物'
union all
